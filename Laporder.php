...
if(isset($_POST['submit'])){
   $P_ID2=$_POST['P_ID1'];
   $sql20="SELECT * FROM `laptopproduct1` WHERE Laptop_ID='$P_ID2';";
    $data=mysqli_query($conn,$sql20);
    $totall=mysqli_num_rows($data);
    while($info=mysqli_fetch_array($data)){
      $pname=$info['Laptop_Name'];
      $imag=$info['img_source'];
      $price=$info['Laptop_Sal'];
      $pruduct=$info['Laptop_pice'];
      //echo $pname;
   }
    if($totall==1){
       // $res=$P_ID;
...
$sqlll="INSERT INTO `mobilesal1` (`Mobile_id`,`p_name`,`img_source`,`P_price`,`Email_id`, `State`, `Distric`, `Village/city`, `Houselocation`, `Pin code`, `Payment type`,`Orderdate`) VALUES ('$P_ID2','$pname','$imag',' $price', '$emaill', 'NAN', '$Stat', '$VC', '$HS', '$PIN', '$Cash','$id2');";
          ...
          if($mass==2){
            ...
            <form method="post" class="px-md-2">
              ...
              <div class="form-outline mb-4">
                <input type="text" id="form3Example1q" name="P_ID1" class="form-control" required />
